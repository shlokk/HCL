...
import moco.hyptorch.pmath as pmath
...
class ToPoincare_constraint(nn.Module):
    r"""
    Module which maps points in n-dim Euclidean space
    to n-dim Poincare ball
    """

    def __init__(self, c, train_c=False, train_x=False, ball_dim=None, riemannian=True, eps=1e-3):
        super(ToPoincare_constraint, self).__init__()
        if train_x:
            if ball_dim is None:
                raise ValueError(
    ...
    def forward(self, x):
        ...
        if self.train_x:
            xp = pmath.project_constraint(pmath.expmap0(self.xp, c=self.c), c=self.c, eps=self.eps)
            return self.grad_fix(pmath.project_constraint(pmath.expmap(xp, x, c=self.c), c=self.c, eps=self.eps))
        return self.grad_fix(pmath.project_constraint(pmath.expmap0(x, c=self.c), c=self.c, eps=self.eps))
...
class ReLU_hyperbolic(nn.Module):
    def __init__(self, c, inplace=False):
        super(ReLU_hyperbolic, self).__init__()
        self.c = c
        self.inplace = inplace

    def forward(self, input):
        out = pmath.expmap0(F.relu(pmath.logmap0(input, c=self.c), inplace=self.inplace), c=self.c)
        return out
